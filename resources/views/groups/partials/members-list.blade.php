                ...
                <li class="border-gray-400 flex flex-row mb-2">
                    <div
                        ...
                        @if (auth()->user()->can('manage-groups') || $data->is_logged_in_user_group_leader)
                            @if(($group->pivot->is_leader && auth()->user()->can('manage-groups')) || $group->pivot->is_leader == false)
                            <form method="POST" action="{{ route('tenant.groups.removeFromGroup', [$data->id, $group->id]) }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
