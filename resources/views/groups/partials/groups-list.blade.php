    ...
    <div class="container flex mx-auto w-full items-center justify-center">
        <ul class="flex flex-col w-full p-4">
            @foreach ($data as $group)
                <a href="{{ route('tenant.groups.view', $group->id) }}">
                    <li class="border-gray-400 flex flex-row mb-2">
                        <div
                            ...
                            @if (auth()->user()->can('manage-groups') || $group->is_logged_in_user_group_leader)
                                <form method="POST" action="{{ route('tenant.groups.delete', $group->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
